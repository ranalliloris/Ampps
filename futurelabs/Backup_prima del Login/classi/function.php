...
{  
    $query="SELECT c.cod_corso, c.link_info, c.nome, c.max_partecipanti, COUNT(c.cod_corso) as iscrizioni
            FROM corso as c
...
function insertPersona($dbconn,$cf,$cognome,$nome,$email,$cellulare,$materia_insegnamento,$classe_concorso,$istituto,$meccanografico)
{
    $values=[       ":cf"=>$cf,
                    ":cognome"=>$cognome,
                    ":nome"=>$nome,
...
//FUNZIONI MODIFICA ISCRIZIONE

function datiPersona($dbconn,$codice_iscrizione)
{
    $query="SELECT *
            FROM persona as p
            WHERE p.cf= (SELECT DISTINCT i.cf
                         FROM iscrizione as i
                         WHERE i.codice_iscrizione=:codice_iscrizione)
            ";
    $parameters=[":codice_iscrizione" => $codice_iscrizione];
    $stmt=$dbconn->query($query,$parameters);
    $row=$stmt->fetch();
    return $row;
}
