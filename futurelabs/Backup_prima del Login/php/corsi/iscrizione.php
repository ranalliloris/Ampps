<?php
    ...
    try
    {
        ...
        {
            insertPersona($dbconn,$cf,$cognome,$nome,$email,$cellulare,$materia_insegnamento,$classe_concorso,$istituto,$meccanografico);
        }
        ...
        if($strCorsiIscritto!="")
        {
            ...
            $html_msg='<h3>Conferma Iscrizioni Corsi Future Labs IIS L. Da Vinci - Firenze</h3>
                        <p>Cognome: '.$cognome.'</p>
                        <p>Nome: '.$nome.'</p>'.$str.'
                        <h5 style="color:#28a745">Codice Iscrizione: '.$codice_iscrizione.'</h5>
                        <p>Il codice iscrizione può essere usato per modificare/cancellare l\'iscrizione a effettuata</p>
                        <p>Per qualunque informazione o richiesta inviare una mail all\'indirizzo
                        mail <a href="mailto:futurelabs@isisdavinci.eu">futurelabs@isisdavinci.eu</a></p>
            ';
            //echo $html_msg;
            //$str=inviaMail($email,"Conferma Iscrizioni Corsi Future Labs IIS L. Da Vinci - Firenze",$html_msg);
            $dati["html"].='<p><b>'.$str.'</b></p>';
        }
